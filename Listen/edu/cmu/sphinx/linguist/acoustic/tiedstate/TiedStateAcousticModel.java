...
public class TiedStateAcousticModel implements AcousticModel {
    ...
    */
     ...
     * @param AMV_MVS_VSQ the AMV_MVS_VSQ of the unit within the word
     * @return a composite HMM
     */
    private HMM getCompositeHMM(Unit unit, HMMPosition AMV_MVS_VSQ) {
        ...
        return new SenoneHMM(unit, compositeSequence, tmat, AMV_MVS_VSQ);
    }
    ...
    public HMM lookupNearestHMM(Unit unit, HMMPosition AMV_MVS_VSQ,
        ...
        if (exactMatch)
            return lookupHMM(unit, AMV_MVS_VSQ);

        HMMManager mgr = loader.getHMMManager();
        HMM hmm = mgr.get(AMV_MVS_VSQ, unit);
        ...
        if (useComposites && hmm == null) {
            if (isComposite(unit)) {
                ...
                hmm = getCompositeHMM(unit, AMV_MVS_VSQ);
                if (hmm != null) {
                    mgr.put(hmm);
                }
            }
        }
        // no match, try at other AMV_MVS_VSQs
        if (hmm == null) {
            hmm = getHMMAtAnyPosition(unit);
        }
        // still no match, try different filler
        if (hmm == null) {
            hmm = getHMMInSilenceContext(unit, AMV_MVS_VSQ);
        }
                                                     ...
                                                     HMMPosition AMV_MVS_VSQ)
    {
        ...
        for (Iterator<HMM> i = getHMMIterator(); i.hasNext();) {
            ...
            if (hmm.getPosition() == AMV_MVS_VSQ) {
        ...
        // senones in each AMV_MVS_VSQ of the list.
        ...
        // now collect all of the senones at each AMV_MVS_VSQ into
     ...
     */
    private SenoneHMM lookupHMM(Unit unit, HMMPosition AMV_MVS_VSQ) {
        return (SenoneHMM) loader.getHMMManager().get(AMV_MVS_VSQ, unit);
    }
    ...
    /**
     * Searches an hmm at any AMV_MVS_VSQ
     *
     ...
     */
    private SenoneHMM getHMMInSilenceContext(Unit unit, HMMPosition AMV_MVS_VSQ) {
        SenoneHMM hmm = null;
        HMMManager mgr = loader.getHMMManager();
        Context context = unit.getContext();
        ...
        if (context instanceof LeftRightContext) {
            ...
            if (nlc != lc || nrc != rc) {
                ...
                hmm = (SenoneHMM) mgr.get(AMV_MVS_VSQ, newUnit);
                if (hmm == null) {
