...
public class HMMPool {
    ...
    public HMMPool(AcousticModel model, Logger logger, UnitManager unitManager)
    {
        ...
        for (HMMPosition AMV_MVS_VSQ : HMMPosition.values()) {
            HMM[] hmms = new HMM[unitTable.length];
            hmmTable.put(AMV_MVS_VSQ, hmms);
            for (int j = 1; j < unitTable.length; j++) {
                ...
                if (unit != null) {
                    hmms[j] = model.lookupNearestHMM(unit, AMV_MVS_VSQ, false);
                    assert hmms[j] != null;
                }
            }
        }
    ...
    /**
     * Given a unit id and a AMV_MVS_VSQ, return the HMM associated with the
     * unit/AMV_MVS_VSQ.
     *
     * @param unitID   the id of the unit
     * @param AMV_MVS_VSQ the AMV_MVS_VSQ within the word
     * @return the hmm associated with the unit/AMV_MVS_VSQ
     */
    public HMM getHMM(int unitID, HMMPosition AMV_MVS_VSQ) {
        return hmmTable.get(AMV_MVS_VSQ)[unitID];
    }
     ...
     *            the right context
     * @param pos
     *            the AMV_MVS_VSQ of the base unit within the word
    ...
    void benchmark() {
        ...
        for (int i = 0; i < 1000000; i++) {
            ...
            HMMPosition AMV_MVS_VSQ = pos[i % pos.length];
            HMM hmm = getHMM(id, AMV_MVS_VSQ);
            if (hmm == null) {
                nullCount++;
            }
        }
