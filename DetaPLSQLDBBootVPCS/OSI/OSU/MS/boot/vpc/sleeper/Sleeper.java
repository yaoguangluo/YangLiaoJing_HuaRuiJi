package OSI.OSU.MS.boot.vpc.sleeper;
import java.io.IOException;
import java.net.Socket;

import ME.sample.App;
import OSI.OSU.MS.boot.vpc.vision.VPCSRequest;
import OSI.OSU.MS.boot.vpc.vision.VPCSResponse;
...
public class Sleeper extends Thread implements Runnable{
	...
	public void run(){
		try{
			OSI.OSU.MS.boot.vpc.controller.RequestRecordController
			...
			OSI.OSU.MS.boot.vpc.controller.RequestFilterController
			...
			OSI.OSU.MS.boot.vpc.controller.RequestFixController
			...
			OSI.OSU.MS.boot.vpc.vision.ForwardVision.getForwardType(vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}
			OSI.OSU.MS.boot.vpc.vision.ForwardVision.forwardToRestMap(app, vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}
			OSI.OSU.MS.boot.vpc.vision.RestMapVision.getResponse(vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}
			OSI.OSU.MS.boot.vpc.vision.RestMapVision.returnResponse(vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}		
		}catch(Exception e){
