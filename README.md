...
###### 5.1 工程函数今天开始准备全部改成UTF8格式, 因为GBK字符缺失问题. 20210409
###### 5.2 思路今天改变, 工程全局准备依旧使用GBK, 然后将所有字符串进行变量化, 然后变量全部放在变量STATBLE文件中用UTF8.
###### 5.3 工程中的 ETL node文件包工程统一用UTF8, 已经矫正. 罗瑶光 20210413
