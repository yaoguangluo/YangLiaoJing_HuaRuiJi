package OSI.OSU.MS.VPC.sleeper;
...
public class Sleeper extends Thread implements Runnable{
	...
	public void run(){
		try{
			OSI.OSU.MSV.PCS.controller.RequestRecordController
			...
			OSI.OSU.MSV.PCS.controller.RequestFilterController
			...
			OSI.OSU.MSV.PCS.controller.RequestFixController
			...
			OSI.OSU.MS.VPC.vision.ForwardVision.getForwardType(vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}
			OSI.OSU.MS.VPC.vision.ForwardVision.forwardToRestMap(app, vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}
			OSI.OSU.MS.VPC.vision.RestMapVision.getResponse(vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}
			OSI.OSU.MS.VPC.vision.RestMapVision.returnResponse(vPCSRequest, vPCSResponse);
			if(vPCSResponse.getSocket().isClosed()) {
				return;
			}		
		}catch(Exception e){
