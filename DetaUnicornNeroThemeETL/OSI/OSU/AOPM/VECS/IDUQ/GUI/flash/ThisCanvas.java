...
import OSI.OSU.OVU.MVU.GUI.nodeEdit.CheckRangeVPS;
import OSI.OSU.OVU.MVU.GUI.nodeEdit.ChooseCheckVPS;
import OSI.OSU.OVU.MVU.GUI.nodeEdit.DrawArrowVPS;
import OSI.OSU.OVU.MVU.GUI.nodeEdit.DrawFlashSide;
import OSI.OSU.OVU.MVU.GUI.nodeEdit.DynamicLineUpdaterVPS;
	...
	public void paint(Graphics g){
		...
		while(node!= null){
			...
			if(node.actionNodeLeft) {
				if(0== isOperation) {
					DrawFlashSide.deleteFlashSide(graphics2D, node.oldx, node.oldy, node.flash++ % 3);
				}else {
					DrawFlashSide.deleteFlashSide(graphics2D, node.oldx, node.oldy, node.flash);
				}
			}
			if(0== isOperation) {
