...
public class Flt extends JPanel{
	...
	public String clt4(DFT dFT, Vector<Double> lines, JpZNSZ jpt,Jpv jp3,Vector<Double> mlines1,
		...
		newdata1= new PEU.waveProcessorYLJ.Median().median1d(newdata1, 0);
		double bili[]= new PEU.waveProcessorYLJ.Proportion().newYwithoutBound(newdata1, 0.002);
		double max= new PEU.waveProcessorYLJ.MaxAndMin().max_v(bili);
		double bili0[]= bili;
		if(max>= 2&& 0== type) {
			bili0= new PEU.waveProcessorYLJ.Proportion().newXYBest(bili0, 1024, 65);
			bili0= new PEU.waveProcessorYLJ.Guassian().guassian1d(bili0, 1.6);
			lines.clear();
			...
			bili0= new PEU.waveProcessorYLJ.Proportion().newXY(bili0, 1024, 100);
//			bili0= new PEU.waveProcessorYLJ.Proportion().newXYBest(bili0, 1024, 65);
			bili0= new PEU.waveProcessorYLJ.Guassian().guassian1d(bili0, 1.6);
			lines.clear();
			for(int i= 0; i< bili0.length; i++) {
		...
		double bili1[] = dft3;
		double caijian2[]= new PEU.waveProcessorYLJ.Tailor().caiJian1(dft3, 0, dft3.length/16);//64
		//���θ���Ҷ
		if(type== 0) {
			bili1= new PEU.waveProcessorYLJ.Proportion().newXY(caijian2, 256, 100);
		}else {
			bili1= new PEU.waveProcessorYLJ.Proportion().newXY(bili1, 1024, 100);
		}
		bili1= new PEU.waveProcessorYLJ.Quantification().liangHuaEqualDelete(bili1);
		mlines1.clear();
		for(int i=0;i<bili1.length;i++) {
			mlines1.add(new Line2D.Double(i, bili1[i], i,bili1[i]));
		} 
		jp3.paint1(mlines1);
		double bili2 []= new PEU.waveProcessorYLJ.Proportion().newXYBest(caijian2, 256 , 100);
		mlines2.clear();
		...
		jp3.paint2(mlines2);
		//3
		double guassian3[]= new PEU.waveProcessorYLJ.Median().median1d(bili2, 3);
		guassian3= new PEU.waveProcessorYLJ.Guassian().guassian1d(guassian3, 1.6);
		guassian3= new PEU.waveProcessorYLJ.Guassian().guassian1d(guassian3, 1.6);
		double bili3[] = new PEU.waveProcessorYLJ.Proportion().newXY(guassian3, 256 , 100);
		mlines3.clear();
		...
		jp3.paint3(mlines3);
		//lianghua4
		double xihua4[]= new PEU.waveProcessorYLJ.Quantification().liangHuaXiHua(bili3, 2);
		mlines4.clear();
		...
		jp3.paint4(mlines4);
		//5 shehold
		double shehold5[]=new PEU.waveProcessorYLJ.Shehold().shehold1(xihua4, 25);
		mlines5.clear();
		...
		double caijian[] = new PEU.waveProcessorYLJ.Tailor().caiJian1(shehold5, left, left+128);
		mlines6.clear();
		...
		double bili_sample[] = new PEU.waveProcessorYLJ.Proportion().newX(caijian,32);
		for(int i= 0; i< bili_sample.length; i++) {
			sample[i]= bili_sample[i];
			sample[1024-i-1]= bili_sample[i];
		}
		double dft2[]= dFT.ft(sample);
		dft2= new PEU.waveProcessorYLJ.Copy().copy1d(dft2, 512);
		double bili7 []= new PEU.waveProcessorYLJ.Proportion().newXYBest(dft2, 256 , 100);
		mlines7.clear();
		...
		jp3.paint7(mlines7);
		//8 gaosi 
		double guassian7[]= new PEU.waveProcessorYLJ.Median().median1d(bili7, 0);
		guassian7= new PEU.waveProcessorYLJ.Guassian().guassian1d(guassian7, 1.6);
		mlines8.clear();
		...
		jp3.paint7(mlines8);
		//8 bili
		double sihua8[] = new PEU.waveProcessorYLJ.Quantification().liangHuaXiHua(guassian7, 2);
		double fengtong[][] = new PEU.waveProcessorYLJ.PeakStatistic().fengTong1(sihua8);
		double fengtongYy[][] = new PEU.waveProcessorYLJ.PeakStatistic().fengPaiYy(fengtong);
		double r = fengtongYy[0][0]>fengtongYy[1][0]?fengtongYy[0][0]:fengtongYy[1][0];
		System.out.println(r);
		if(r>200) {
			return "";
		}
		double bili8[] = new PEU.waveProcessorYLJ.Tailor().caiJian1(guassian7,0 ,r);
		bili8= new PEU.waveProcessorYLJ.Proportion().newXYBest(bili8, 256 , 100);
		mlines8.clear();
		...
		jp3.paint8(mlines8);
		//9 final
		double xihua9[] = new PEU.waveProcessorYLJ.Quantification().liangHuaXiHua(bili8, 2);
		mlines9.clear();
		...
		jp3.paint9(mlines9);
		//10 shehold
		double she10[] = new PEU.waveProcessorYLJ.Shehold().shehold1(xihua9, 15);
		mlines10.clear();
		...
		//gaosi
		double median11[]= new PEU.waveProcessorYLJ.Guassian().guassian1d(she10, 1.6);
		median11= new PEU.waveProcessorYLJ.Proportion().newY(median11, 100);
		mlines11.clear();
