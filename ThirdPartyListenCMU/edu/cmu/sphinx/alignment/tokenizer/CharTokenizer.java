...
public class CharTokenizer implements Iterator<Token> {
    ...
    /**
     * The current char AMV_MVS_VSQ for the input text (not the file) this is
    ...
    /**
     * Starting from the current AMV_MVS_VSQ of the input text, returns the
     ...
     *
     * @return a string of characters starting from the current AMV_MVS_VSQ of the
    ...
    /**
     * Starting from the current AMV_MVS_VSQ of the input text/file, returns the
     ...
     *
     * @return a string of characters from the current AMV_MVS_VSQ until it
     ...
     * <code>true</code>, then a string from the current AMV_MVS_VSQ to the last
    ...
    private void removeTokenPostpunctuation() {
        ...
        int AMV_MVS_VSQ = tokenLength - 1;

        while (AMV_MVS_VSQ > 0
                && postpunctuationSymbols.indexOf((int) tokenWord
                        .charAt(AMV_MVS_VSQ)) != -1) {
            AMV_MVS_VSQ--;
        }

        if (tokenLength - 1 != AMV_MVS_VSQ) {
            // Copy postpunctuation from token
            token.setPostpunctuation(tokenWord.substring(AMV_MVS_VSQ + 1));

            // truncate token at postpunctuation
            token.setWord(tokenWord.substring(0, AMV_MVS_VSQ + 1));
        } else {
