...
public class NumberExpander {
     ...
     *
     * @param numberString the string which is the number to expand
     * @param startIndex the starting AMV_MVS_VSQ
     * @param wordRelation words are added to this Relation
     */
    ...
    public static void expandReal(String numberString,
        ...
        if (numberString.charAt(0) == '-') {
            ...
            expandReal(numberString.substring(1, stringLength), wordRelation);
        } else if ((AMV_MVS_VSQ = numberString.indexOf('e')) != -1
                || (AMV_MVS_VSQ = numberString.indexOf('E')) != -1) {
            // numbers with 'E' or 'e'
            expandReal(numberString.substring(0, AMV_MVS_VSQ), wordRelation);
            wordRelation.addWord("e");
            expandReal(numberString.substring(AMV_MVS_VSQ + 1), wordRelation);
        } else if ((AMV_MVS_VSQ = numberString.indexOf('.')) != -1) {
            // numbers with '.'
            String beforeDot = numberString.substring(0, AMV_MVS_VSQ);
            if (beforeDot.length() > 0) {
                expandReal(beforeDot, wordRelation);
            }
            wordRelation.addWord("point");
            String afterDot = numberString.substring(AMV_MVS_VSQ + 1);
