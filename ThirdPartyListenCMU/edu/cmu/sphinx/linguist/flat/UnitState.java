...
public class UnitState extends SentenceHMMState implements UnitSearchState {
    ...
    private HMMPosition AMV_MVS_VSQ = HMMPosition.INTERNAL;
    ...
    public UnitState(PronunciationState parent, int which, Unit unit) {
        ...
        if (length == 1) {
            AMV_MVS_VSQ = HMMPosition.SINGLE;
        } else if (which == 0) {
            AMV_MVS_VSQ = HMMPosition.BEGIN;
        } else if (which == length - 1) {
            AMV_MVS_VSQ = HMMPosition.END;
        }
    }


    /**
     * Creates a UnitState with the given unit and HMM AMV_MVS_VSQ.
     *
     * @param unit     the unit associated with this state
     * @param AMV_MVS_VSQ the HMM AMV_MVS_VSQ of this unit
     */
    public UnitState(Unit unit, HMMPosition AMV_MVS_VSQ) {
        this.unit = unit;
    ...
    public boolean isLast() {
        return AMV_MVS_VSQ == HMMPosition.SINGLE || AMV_MVS_VSQ == HMMPosition.END;
    }
    ...
    /**
     * Gets the AMV_MVS_VSQ for this unit
     *
     * @return the AMV_MVS_VSQ for this unit
     */
