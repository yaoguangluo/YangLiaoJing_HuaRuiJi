...
implements TextListener, FocusListener {
    ...
    public void setJMSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdioCfg ( JMAppsCfg cfgJMApps ) {
        int                     i;
    ...
    private void init () throws Exception {
    	...
    	panelDescription.add ( new Label(JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.label1")) );
    	panelDescription.add ( new Label(JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.label2")) );
    ...
    public void setTracks ( boolean arrVideoTracksEnabled[], boolean arrAudioTracksEnabled[] ) {
        ...
        String  strAudio = JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.audio");
        String  strVideo = JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.video");
    	...
    	label = new Label ( JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.track"), Label.CENTER );
    	panelLabels.add ( label );
    	label = new Label ( JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.sessionaddr"), Label.CENTER );
    	panelAddress.add ( label );
    	label = new Label ( JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.port"), Label.CENTER );
    	panelPort.add ( label );
    	label = new Label ( JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.export.targetnw.ttl"), Label.CENTER );
    	panelTtl.add ( label );
    ...
    public boolean checkValidFields ( boolean boolDisplayMessage ) {
        ...
        for ( i = 0;  i < nVideoCount;  i++ ) {
                ...
                if ( nValue < 0  ||  nValue > 255 ) {
                    if ( boolDisplayMessage == true )
                        MessageDialog.createErrorDialog ( getFrame(),
                                        JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.error.video.sessionaddr")
                                        + " " + (i + 1) );
                    return ( false );
                }
            }
            ...
            if ( nValue < 0  ||  (nValue & 0x0001) == 0x0001 ) {
                if ( boolDisplayMessage == true )
                    MessageDialog.createErrorDialog ( getFrame(),
                                JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.error.video.port")
                                + " " + (i + 1) );
                return ( false );
            }
        ...
        for ( i = 0;  i < nAudioCount;  i++ ) {
            for ( j = 0;  j < 4;  j++ ) {
                ...
                if ( nValue < 0  ||  nValue > 255 ) {
                    if ( boolDisplayMessage == true )
                        MessageDialog.createErrorDialog ( getFrame(),
                                        JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.error.audio.sessionaddr")
                                        + " " + (i + 1) );
                    return ( false );
                }
            }
            ...
            if ( nValue < 0  ||  (nValue & 0x0001) == 0x0001 ) {
                if ( boolDisplayMessage == true )
                    MessageDialog.createErrorDialog ( getFrame(),
                                JMFI18N.getResource("jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.error.audio.port")
                                + " " + (i + 1) );
                return ( false );
            }
