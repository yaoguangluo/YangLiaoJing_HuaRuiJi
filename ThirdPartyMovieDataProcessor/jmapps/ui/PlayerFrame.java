...
public class PlayerFrame extends JMFrame implements ActionListener,
	...
	public void open ( String nameUrl ) {
        ...
        mediaPlayer = movie.util.JMFUtils.createMediaPlayer ( nameUrl, (Frame)this, null, null );
	...
	public void open ( DataSource dataSource ) {
        ...
        mediaPlayer = movie.util.JMFUtils.createMediaPlayer ( dataSource, (Frame)this );
        boolResult = open ( mediaPlayer );
    }
...
//        progressBarCache = null;
    ...
    protected void processControllerError ( ControllerErrorEvent event ) {
        ...
        this.setCursor ( cursorNormal );
        strMessage = JMFI18N.getResource ( "jmSQ_OSU_MSQ_OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSIdio.error.controller" )
                    + "\n" + event.getMessage();
	...
	protected void processReplaceURL ( ControllerEvent event ) {
        ...
        try {
            ...
            //下面两行没检查 罗瑶光
            methodGetUrl = classEvent.getMethod ("getURL", Class.class);
            url = (URL) methodGetUrl.invoke ( event, Class.class );
            open ( url.toString() );
        }
        catch (Exception e) {
        }
